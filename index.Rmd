...
For some content I will provide recommended reading.  These are texts are mostly those I've benefitted from or I would think would be useful to others, especially those just starting out with analysis and/or at an intermediate stage.  They are not meant to be a list seminal works on the subject, though some might be.
...
m = prophet::prophet(Series.df, growth = 'linear', n.changepoints = 1)
m$changepoints                                 # date of estimated changepoint
forecast = predict(m, Series.df)
plot(m, fcst=forecast)
```


## Time series list of packages

xts, zoo, forecast, prophet, lubridate
