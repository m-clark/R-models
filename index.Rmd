...
In what follows, you will not see any model which I have not run on my own, if not many, many times. Like a yuge amount. Seriously.
...
For some content I will provide recommended reading.  These are texts are mostly those I've benefitted from or I would think would be useful to others, especially those just starting out with analysis, at an intermediate stage, or even coming to R from another language.  They are not meant to be a list seminal works on the subject, though some might be.
...
There are several UseR! series entries for graphical models and network analysis.
...
```{r sp_getgraph, eval=TRUE, echo=FALSE, cache=FALSE}
